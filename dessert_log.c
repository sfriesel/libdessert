 ...
 * @arg level loglevel from <syslog.h>
 * @arg *func function name called from
...
void _dessert_log(int level, const char* func, const char* file, int line, const char *fmt, ...) {
    ...
    if(_dessert_logflags | (_DESSERT_LOGFLAG_LOGFILE|_DESSERT_LOGFLAG_STDERR|_DESSERT_LOGFLAG_RBUF) ) {
        ...
        time(&ldi);
        localtime_r(&ldi, &ldd);
        snprintf(lds, 26, "%04d-%02d-%02d %02d:%02d:%02d%+05.1f ", ldd.tm_year+1900, ldd.tm_mon+1, ldd.tm_mday, ldd.tm_hour, ldd.tm_min, ldd.tm_sec, (double) ldd.tm_gmtoff/3600);
        ...
        if(32 + buf_slen + lf_slen > 80) {
            if(_dessert_logflags & _DESSERT_LOGFLAG_LOGFILE && dessert_logfd != NULL)
            ...
            if(_dessert_logflags & _DESSERT_LOGFLAG_RBUF && rbuf_line != NULL)
                snprintf(rbuf_line, DESSERT_LOGLINE_MAX, "%s%s%s%s", lds, lt, buf, lf);
        }
...
int _dessert_cli_logging_ringbuffer(struct cli_def *cli, char *command, char *argv[], int argc)
{
    ...
    if (newlen == 0) {
        cli_print(cli, "will not set buffer length to 0 - use no logging ringbuffer instead\n");
        return CLI_ERROR;
    }
    ...
    if (newlen > _dessert_logrbuf_len) {
        _dessert_logrbuf=realloc(_dessert_logrbuf, newlen*DESSERT_LOGLINE_MAX*sizeof(char));
    ...
    /* make logging buffer smaller - pain in the ass */
        ...
        if(_dessert_logrbuf_cur>newlen) {
            memmove(_dessert_logrbuf, _dessert_logrbuf+(DESSERT_LOGLINE_MAX*(_dessert_logrbuf_cur-newlen)) ,newlen*DESSERT_LOGLINE_MAX*sizeof(char));
...
int _dessert_cli_cmd_logging(struct cli_def *cli, char *command, char *argv[], int argc)
{
    ...
    if(_dessert_logrbuf_len<1) {
        cli_print(cli, "logging to ringbuffer is disables - use \"logging ringbuffer [int]\" in config-mode first");
        pthread_rwlock_unlock(&_dessert_logrbuf_len_lock);
        return CLI_ERROR;
    }
 ...
 * @arg opts or-ed flags - @see DESSERT_LOG_*
**/
int dessert_logcfg(uint16_t opts)
{
    ...
    if((opts & DESSERT_LOG_STDERR) && !(opts & DESSERT_LOG_NOSTDERR) && !(_dessert_status & _DESSERT_STATUS_DAEMON)){
        _dessert_logflags |= _DESSERT_LOGFLAG_STDERR;
    } else if((!(opts & DESSERT_LOG_STDERR) && (opts & DESSERT_LOG_NOSTDERR)) || (_dessert_status & _DESSERT_STATUS_DAEMON)) {
        _dessert_logflags &= ~_DESSERT_LOGFLAG_STDERR;
    }
    if((opts & DESSERT_LOG_FILE) && !(opts & DESSERT_LOG_NOFILE) && dessert_logfd != NULL){
        _dessert_logflags |= _DESSERT_LOGFLAG_LOGFILE;
    } else if((!(opts & DESSERT_LOG_FILE) && (opts & DESSERT_LOG_NOFILE)) || dessert_logfd == NULL) {
        _dessert_logflags &= ~_DESSERT_LOGFLAG_LOGFILE;
    }
